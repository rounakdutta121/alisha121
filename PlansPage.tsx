...
const PlansPage: React.FC<PlansPageProps> = ({ user }) => {
  ...
  const startCheckout = async () => {
    ...
    setUpgrading(true);
    try {
      console.log('Initiating checkout via Vercel for:', user.email);
      ...
      if (data?.url) {
        ...
        // Direct redirect using standard window location
        window.location.assign(data.url);
      } else {
        throw new Error(data?.error || 'Payment server failed to provide a checkout URL.');
      }
    } catch (err: any) {
      console.error('ALISHA Checkout Exception:', err);

      let msg = "Unable to initialize secure payment. Please try again later.";
      if (err.message === 'Failed to fetch') {
        msg = "The payment server is unreachable. Please check your internet connection or try again in a few moments.";
      } else if (err.message) {
        msg = err.message;
      }
          ...
          <div className="flex flex-col gap-4">
            {!isPremium ? (
              ...
              <>
                ...
                <button className="px-8 py-4 bg-slate-900/50 hover:bg-slate-800 text-slate-400 rounded-2xl font-bold transition-all border border-slate-800 text-sm">
